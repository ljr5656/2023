...
export class Group extends FabricObject {
  ...
  _restoreObjectState(object): Group {
    let groupLeft = this.get('left'),
      ...
      rotatedTop =
        Math.cos(groupAngle) * object.get('top') +
        Math.sin(groupAngle) * object.get('left'),
      rotatedLeft =
        -Math.sin(groupAngle) * object.get('top') +
        Math.cos(groupAngle) * object.get('left');
  ...
  private _set(key: string, value): Group {
    this[key] = value;
    return this;
  }
